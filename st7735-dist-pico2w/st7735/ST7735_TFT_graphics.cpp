...
// buffer MUST be of size _widthTFT * _heightTFT * sizeof(uint16_t)
void ST7735_TFT_graphics::TFTdrawBitmap16FullScreen(uint16_t *buffer)
{
	TFTsetAddrWindow(0, 0, 0 + _widthTFT - 1, 0 + _heightTFT - 1);
	spiWriteDataBuffer((uint8_t*)buffer, _widthTFT * _heightTFT * sizeof(uint16_t));
}
